...
With more than one app in `apps/` your commands must specify which one: `bun run dev weather`.

Screenshot for the store listing:

```sh
bun run shot weather            # grabs what is on the screen
bun run shot weather --replace  # overwrite
```
...
```sh
bun run bump weather patch -m "Fix the wind direction arrow"
git commit -am "weather: fix the wind direction arrow" && git push
```
